package com.wire.utils;
/**
 * Classe dans laquelle toutes les valeurs de paramétrage sont définies.
 * 
 * @author david
 *	
 *
 */
public class Values {

	public final static String WS_CALLER_ID="ipponTech";
	public final static String WS_CALLER_SECRET="secret";
	public final static String LABEL_WS_CALLER_ID ="wsCallerId";
	public final static String LABEL_WS_CALLER_SECRET ="wsCallerSecret";
	public final static String LABEL_HASH ="hash";
	public final static String LABEL_LOGIN ="identifiant";
	public final static String LABEL_PASSWORD ="password";
	public final static String LABEL_WS_TIMESTAMP ="timestamp";
	public final static String LABEL_NAME = "nom";
	public final static String LABEL_FST_NAME = "prenom";
	public final static String LABEL_BIRTHDATE = "dateNaissance";
	public final static String LABEL_STREETNUM = "numeroRue";
	public final static String LABEL_STREETNAME = "nomRue";
	public final static String LABEL_CITY = "ville";
	public final static String LABEL_ZIPCODE = "codePostal";
	public final static String LABEL_COUNTRY = "pays";
	public final static String LABEL_PHONE_NUM = "tel";
	public final static String LABEL_MAIL = "email";
	public final static String LABEL_PHOTO_STREAM = "photoStream";
	public final static String LABEL_PHOTO_URL = "photoURL";
	
	public final static String LABEL_SESSION_TOKEN = "sessionToken";
	
	public final static String URL_LOGIN="http://10.0.2.2:8080/ab_service_mgr/api/mobile/login/";
	public final static String URL_LOGOUT="http://10.0.2.2:8080/ab_service_mgr/api/mobile/logout/";
	public final static String REQUEST_METHOD ="POST";
	public final static String LABEL_CONTENT_TYPE ="Content-Type";
	public final static String CONTENT_TYPE ="application/json";
	
	public final static String URL_HELP_WEB_VIEW = "http://erems.free.fr";
	public final static String URL_HELP_TERMS_CONDITIONS_VIEW = "http://google.com";

	public final static String ERROR_MESSAGE_CONNECTION_RESEAU = "Vous n'êtes pas connecté.";
	public final static String ERROR_MESSAGE_CONNECTION_SERVEUR = "Identifiant et/ou Mot de passe incorrects. ";
	
	
	public final static String ERROR_TITLE_EMPTY_PARAMETERS = "Erreur Connexion";
	public final static String ERROR_MESSAGE_EMPTY_PARAMETERS =" Veuillez renseigner vos identifiants et mot de passe";
	public final static String ERROR_MESSAGE_WRONG_PARAMETERS = "Les identifiant et/ou mot de passe sont incorrects";
	public static final String LABEL_STATUS = "status";
}

